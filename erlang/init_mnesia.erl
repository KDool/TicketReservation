%% init_mnesia.erl (correct: create_schema ONCE for all nodes)
-module(init_mnesia).
-export([bootstrap/3]).

bootstrap(Nodes, MarkerPath, SeatTable) ->
    ...
    %% 6) Create replicated table
    Attrs = [key, state, user_id, hold_id, expires_at, order_id],
    Def = [{attributes, Attrs}, {type, set}, {disc_copies, Nodes}],
    CT = mnesia:create_table(SeatTable, Def),
    io:format("create_table(~p) => ~p~n", [SeatTable, CT]),
    case CT of
        {atomic, ok} -> ok;
        {aborted, {already_exists, SeatTable}} -> ok;
        Other3 -> throw({create_table_failed, Other3})
    end,

    %% 7) Wait table everywhere
    lists:foreach(fun(N) ->
        R = rpc:call(N, mnesia, wait_for_tables, [[SeatTable], 30000]),
        io:format("wait_for_tables(~p) on ~p => ~p~n", [SeatTable, N, R]),
        case R of
            ok -> ok;
            Other4 -> throw({table_not_ready, N, Other4})
        end
    end, Nodes),
